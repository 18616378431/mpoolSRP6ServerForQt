...
void BigNumber::SetBinary(const uint8 *bytes, int32 len, bool littleEndian)
{
    if (littleEndian)
    {
...
bool BigNumber::SetHexStr(const char *str)
{
...
int BigNumber::CompareTo(const BigNumber &bn) const
{
...
void BigNumber::GetBytes(uint8 *buf, size_t bufsize, bool littleEndian) const
{
    ...
    ASSERT(nBytes >= 0, "BigNum has negative number bytes");
    std::size_t numBytes = static_cast<std::size_t>(nBytes);
    ASSERT(numBytes <= bufsize, "busize is to small");
    if (numBytes < bufsize)
